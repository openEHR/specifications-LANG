[cols="2,3", frame=none, grid=rows]
|===
2+^h|*BMM_MODEL*

2+a|Definition of the root of a BMM model (along with what is inherited from `BMM_SCHEMA_CORE`).

2+|Inherits: xref:{class_bmm_declaration}[BMM_DECLARATION], xref:{class_bmm_package_container}[BMM_PACKAGE_CONTAINER], xref:{class_bmm_model_metadata}[BMM_MODEL_METADATA]

2+h|*Attributes*

|*name*: `link:/classes/String[String] ` [1..1]
a|Name of this model element. +
_Inherited from xref:{class_bmm_declaration}[BMM_DECLARATION]_

|*documentation*: `link:/classes/Hash[Hash] <link:/classes/String[String] ,link:/classes/Any[Any] >` [0..1]
a|Optional documentation of this element, as a keyed list.

It is strongly recommended to use the following key /type combinations for the relevant purposes:

* `"purpose": String`
* `"keywords": List<String>`
* `"use": String`
* `"misuse": String`
* `"references": String`

Other keys and value types may be freely added. +
_Inherited from xref:{class_bmm_declaration}[BMM_DECLARATION]_

|*scope*: `xref:{class_bmm_package_container}[BMM_PACKAGE_CONTAINER]` [1..1]
a|Model element within which a referenceable element is known. +
_Inherited from xref:{class_bmm_package_container}[BMM_PACKAGE_CONTAINER]_

|*extensions*: `link:/classes/Hash[Hash] <link:/classes/String[String] ,link:/classes/Any[Any] >` [0..1]
a|Optional meta-data of this element, as a keyed list. May be used to extend the meta-model. +
_Inherited from xref:{class_bmm_declaration}[BMM_DECLARATION]_

|*packages*: `link:/classes/Hash[Hash] <link:/classes/String[String] ,xref:{class_bmm_package}[BMM_PACKAGE]>` [0..1]
a|Child packages; keys all in upper case for guaranteed matching. +
_Inherited from xref:{class_bmm_package_container}[BMM_PACKAGE_CONTAINER]_

|*rm_publisher*: `link:/classes/String[String] ` [1..1]
a|Publisher of model expressed in the schema. +
_Inherited from xref:{class_bmm_model_metadata}[BMM_MODEL_METADATA]_

|*rm_release*: `link:/classes/String[String] ` [1..1]
a|Release of model expressed in the schema as a 3-part numeric, e.g. "3.1.0" . +
_Inherited from xref:{class_bmm_model_metadata}[BMM_MODEL_METADATA]_

|*class_definitions*: `link:/classes/Hash[Hash] <link:/classes/String[String] ,xref:{class_bmm_class}[BMM_CLASS]>` [0..1]
a|All classes in this model, keyed by type name.

|*used_models*: `link:/classes/List[List] <xref:{class_bmm_model}[BMM_MODEL]>` [0..1]
a|List of other models 'used' (i.e. 'imported' by this model). Classes in the current model may refer to classes in a used model by specifying the other class's `_scope_` meta-attribute.

2+h|*Functions*

|*is_root_scope* (): `link:/classes/Boolean[Boolean] ` +
 +
__Post_result__: `Result = (scope = self)` [1..1]
a|True if this declaration entity is the root of the declaration hierarchy. +
_Inherited from xref:{class_bmm_declaration}[BMM_DECLARATION]_

|*package_at_path* ( +
a_path: `link:/classes/String[String] [1]` +
): `xref:{class_bmm_package}[BMM_PACKAGE]` [0..1]
a|Package at the path `_a_path_`. +
_Inherited from xref:{class_bmm_package_container}[BMM_PACKAGE_CONTAINER]_

|*do_recursive_packages* ( +
action: `xref:{class_el_procedure_agent}[EL_PROCEDURE_AGENT][1]` +
): `void` [1..1]
a|Recursively execute `_action_`, which is a procedure taking a `BMM_PACKAGE` argument, on all members of packages. +
_Inherited from xref:{class_bmm_package_container}[BMM_PACKAGE_CONTAINER]_

|*has_package_path* ( +
a_path: `link:/classes/String[String] [1]` +
): `link:/classes/Boolean[Boolean] ` [1..1]
a|True if there is a package at the path `_a_path_`; paths are delimited with delimiter `\{BMM_DEFINITIONS}_Package_name_delimiter_`. +
_Inherited from xref:{class_bmm_package_container}[BMM_PACKAGE_CONTAINER]_

|*model_id* (): `link:/classes/String[String] ` [1..1]
a|Identifier of this model, lower-case, formed from:

`<rm_publisher>_<model_name>_<rm_release>`

E.g. `"openehr_ehr_1.0.4"`.

|*class_definition* ( +
a_name: `link:/classes/String[String] [1]` +
): `xref:{class_bmm_class}[BMM_CLASS]` [1..1]
a|Retrieve the class definition corresponding to `_a_type_name_` (which may contain a generic part).

|*type_definition* (): `xref:{class_bmm_class}[BMM_CLASS]` [1..1]
a|Retrieve the class definition corresponding to `_a_type_name_`. If it contains a generic part, this will be removed if it is a fully open generic name, otherwise it will remain intact, i.e. if it is an effective generic name that identifies a `BMM_GENERIC_CLASS_EFFECTIVE`.

|*has_class_definition* ( +
a_class_name: `link:/classes/String[String] [1]` +
): `link:/classes/Boolean[Boolean] ` [1..1]
a|True if `_a_class_name_` has a class definition in the model.

|*has_type_definition* ( +
a_type_name: `link:/classes/String[String] [1]` +
): `link:/classes/Boolean[Boolean] ` [1..1]
a|True if `_a_type_name_` is already concretely known in the system, including if it is generic, which may be open, partially open or closed.

|*enumeration_definition* ( +
a_name: `link:/classes/String[String] [1]` +
): `xref:{class_bmm_enumeration}[BMM_ENUMERATION]` [1..1]
a|Retrieve the enumeration definition corresponding to `_a_type_name_`.

|*primitive_types* (): `link:/classes/List[List] <link:/classes/String[String] >` [0..1]
a|List of keys in `_class_definitions_` of items marked as primitive types.

|*enumeration_types* (): `link:/classes/List[List] <link:/classes/String[String] >` [0..1]
a|List of keys in `_class_definitions_` of items that are enumeration types.

|*property_definition* (): `xref:{class_bmm_property}[BMM_PROPERTY]` [1..1]
a|Retrieve the property definition for `_a_prop_name_` in flattened class corresponding to `_a_type_name_`.

|*ms_conformant_property_type* ( +
a_bmm_type_name: `link:/classes/String[String] [1]`, +
a_bmm_property_name: `link:/classes/String[String] [1]`, +
a_ms_property_name: `link:/classes/String[String] [1]` +
): `link:/classes/Boolean[Boolean] ` [1..1]
a|True if `_a_ms_property_type_` is a valid 'MS' dynamic type for `_a_property_` in BMM type `_a_bmm_type_name_`. 'MS' conformance means 'model-semantic' conformance, which abstracts away container types like `List<>`, `Set<>` etc and compares the dynamic type with the relation target type in the UML sense, i.e. regardless of whether there is single or multiple containment.

|*property_definition_at_path* (): `xref:{class_bmm_property}[BMM_PROPERTY]` [1..1]
a|Retrieve the property definition for `_a_property_path_` in flattened class corresponding to `_a_type_name_`.

|*class_definition_at_path* ( +
a_type_name: `link:/classes/String[String] [1]`, +
a_prop_path: `link:/classes/String[String] [1]` +
): `xref:{class_bmm_class}[BMM_CLASS]` [1..1]
a|Retrieve the class definition for the class that owns the terminal attribute in `a_prop_path` in flattened class corresponding to `a_type_name`.

|*all_ancestor_classes* ( +
a_class: `link:/classes/String[String] [1]` +
): `link:/classes/List[List] <link:/classes/String[String] >` [0..1]
a|Return all ancestor types of `_a_class_name_` up to root class (usually `Any`, `Object` or something similar). Does  not include current class. Returns empty list if none.

|*is_descendant_of* ( +
a_class_name: `link:/classes/String[String] [1]`, +
a_parent_class_name: `link:/classes/String[String] [1]` +
): `link:/classes/Boolean[Boolean] ` [1..1]
a|True if `_a_class_name_` is a descendant in the model of `_a_parent_class_name_`.

|*type_conforms_to* ( +
a_desc_type: `link:/classes/String[String] [1]`, +
an_anc_type: `link:/classes/String[String] [1]` +
): `link:/classes/Boolean[Boolean] ` [1..1]
a|Check conformance of `_a_desc_type_` to `_an_anc_type_`; the types may be generic, and may contain open generic parameters like 'T' etc. These are replaced with their appropriate constrainer types, or Any during the conformance testing process.

Conformance is found if:

* [base class test] types are non-generic, and either type names are identical, or else `_a_desc_type_` has `_an_anc_type_` in its ancestors;
* both types are generic and pass base class test; number of generic params matches, and each generic parameter type, after 'open parameter' substitution, recursively passes; `_type_name_conforms_to_` test
* descendant type is generic and ancestor type is not, and they pass base classes test.

|*subtypes* ( +
a_type: `link:/classes/String[String] [1]` +
): `link:/classes/List[List] <link:/classes/String[String] >` [1..1]
a|Generate type substitutions for the supplied type, which may be simple, generic (closed, open or partially open), or a container type. In the generic and container cases, the result is the permutation of the base class type and type substitutions of all generic parameters.

.Parameters +
[horizontal]
`_a_type_`:: Name of a type.

|*any_class_definition* (): `xref:{class_bmm_simple_class}[BMM_SIMPLE_CLASS]` [1..1]
a|`BMM_SIMPLE_CLASS` instance for the `Any` class. This may be defined in the BMM schema, but if not, use `BMM_DEFINITIONS._any_class_`.

|*any_type_definition* (): `xref:{class_bmm_simple_type}[BMM_SIMPLE_TYPE]` [1..1]
a|`BMM_SIMPLE_TYPE` instance for the `Any` type.

|*boolean_type_definition* (): `xref:{class_bmm_simple_type}[BMM_SIMPLE_TYPE]` [1..1]
a|`BMM_SIMPLE_TYPE` instance for the `Boolean` type.

2+h|*Invariants*

2+a|__Inv_top_level_scope__: `scope = self`

|===
