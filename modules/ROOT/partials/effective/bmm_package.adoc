...
2+|Inherits: xref:{class_bmm_declaration}[BMM_DECLARATION], xref:{class_bmm_package_container}[BMM_PACKAGE_CONTAINER]
...
|*classes*: `link:/classes/List[List] <xref:{class_bmm_class}[BMM_CLASS]>` [0..1]
...
|*root_classes* (): `link:/classes/List[List] <xref:{class_bmm_class}[BMM_CLASS]>` [0..1]
