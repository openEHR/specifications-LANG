...
2+|Inherits: xref:{class_el_expression}[EL_EXPRESSION], xref:{class_el_simple}[EL_SIMPLE], xref:{class_el_operator}[EL_OPERATOR]
...
_Inherited from xref:{class_el_operator}[EL_OPERATOR]_
...
|*left_operand*: `xref:{class_el_simple}[EL_SIMPLE]` [1..1]
a|Left operand node.

|*right_operand*: `xref:{class_el_terminal}[EL_TERMINAL]` [1..1]
