...
2+|Inherits: xref:{class_bmm_declaration}[BMM_DECLARATION], xref:{class_bmm_module}[BMM_MODULE], xref:{class_bmm_class}[BMM_CLASS], xref:{class_bmm_simple_class}[BMM_SIMPLE_CLASS]
...
|*item_values*: `link:/classes/List[List] <xref:{class_bmm_primitive_value}[BMM_PRIMITIVE_VALUE]>` [0..1]
