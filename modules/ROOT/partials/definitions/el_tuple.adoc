...
h|*Inherit*
2+|`xref:{class_el_instance_ref}[EL_INSTANCE_REF]`
...
h|*0..1*
|*items*: `link:/classes/List[List] <xref:{class_el_tuple_item}[EL_TUPLE_ITEM]>`
a|Items in the tuple, potentially with names. Typical use is to represent an argument list to routine call.

h|*1..1*
|*type*: `xref:{class_bmm_tuple_type}[BMM_TUPLE_TYPE]`
...
h|*1..1 +
(effected)*
|*eval_type* (): `xref:{class_bmm_type}[BMM_TYPE]`
a|Return `_type_`.
|===
