...
In the above model, the sub-types of `BMM_LITERAL_VALUE` are limited to those that are generically applicable in any other part of the model. Other value-specific `BMM_VALUE` descendants are also defined within specific sub-packages.

== Meaning of BMM_LITERAL_VALUE Meta-type
