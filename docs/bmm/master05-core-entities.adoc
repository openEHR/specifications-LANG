...
The simplest example of the former case is the typing of properties, as shown in the instance diagram below.

[.text-center]
.BMM entities
image::{diagrams_uri}/bmm_entities.svg[id=bmm_entities, align="center"]

In the latter case, a type is understood as the _generator_ of instances. In a static model, types are references. For simple types, they refer to the corresponding simple class definition, but for generic types, they refer to a usage of a generic class definition with particular type substitutions. A generic class may generate numerous generic types.
