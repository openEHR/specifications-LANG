...
In BMM, a _model_ is understood as a set of _package_ and _module_ definitions that are developed and maintained as a unit, by some organisation, and usually having an associated hierarchical namespace. A BMM model is structured in the same general way as a UML model, i.e. with a hierarchical package containment structure and a set of module definitions, where the most common type of module is the _class_.  
