...
Features are arranged within a class in _feature groups_. By default there is at least one feature group called `"feature"`. Feature groups may be given any name, and are usually used to group features on the basis of things like:
...
Each feature group may have its own meta-data in thee `_properties_` attribute, and may also set default visibility for all features in the group.
