...
In the above model, multiple inheritance is used in order to retain the design intention of the model. In many modern languages, multiple inheritance of classes containing attributes is not possible. Accordingly, to aid implementation in these languages, the diagram distinguishes the two inheritance hierarchies as follows:

* the 'true' inheritance hierarchy under the class `BMM_TYPED`, corresponding to the type taxonomy illustrated above (green);
* a second hierarchy used to obtain structural elements, under `BMM_DECLARAION`, corresponding to the feature taxonomy illustrated above.
...
* define the BMM classes `BMM_ROUTINE`, `BMM_CLASS_SCOPED`, `BMM_CLASS_ENTITY`, `BMM_DECLARATION` etc as interfaces, with the attributes (such as `BMM_ROUTINE._parameters_`) declared as methods and declare the inheritance as interface inheritance (e.g. `implements` in Java);
