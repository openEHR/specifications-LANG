...
At runtime a constant has a value, which may be of any type, including complex types, and which is set using its `_generator_` expression, which is an instance of a concrete descendant of the meta-type `EL_INSTANCE_REF`. In the common degenerate case, `_generator_` consists simply of a literal value, but may also be a function call or other expression valid in the context of the scoping class. This latter capability is the means by which _computed constants_ are supported.
