...
The abstract classes in this model that define the taxonomy of meta-types are as follows:

* `BMM_FEATURE`: a declared model element referencable by name within a scope and having a _signature_;
* `BMM_ROUTINE`: a feature whose definition is a compututation;
* `BMM_TYPED`: an entity that may be instantiated at execution time;
* `BMM_VALUE`: a model element representing a literal value;
* `BMM_EVALUABLE`: any entity that can be evaluated at execution time to produce a value;
...
All formally defined model elements (i.e. instances of the meta-type `BMM_FEATURE`) have a _scope_, represented by the `_scope_` meta-attribute. The scope meta-type indicates the kind of model element within which the current element is scoped, and the value 
...
Functions and procedures are kinds of _routine_, which are computational features of a class. In BMM, they are represented, along with constituent parts, namely parameters and local variables by descendants of the class `BMM_FEATURE`.
