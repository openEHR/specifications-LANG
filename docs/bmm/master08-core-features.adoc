...
The two types of routines, i.e. functions and procedures include zero or more formal parameters in their signatures, with functions also having a return type. The standard taxonomy of signature types is illustrated below, with each form being controlled by invariants stated in the relevant containing meta-classes (e.g. `BMM_FUNCION`, representing a function definition has an invariant requiring `signature.result /= Void`).

[.text-center]
.Signature taxonomy
image::{diagrams_uri}/signature_taxonomy.svg[id=signature_taxonomy, align="center", width="50%"]

The general form of routine signatures is thus as follows:
