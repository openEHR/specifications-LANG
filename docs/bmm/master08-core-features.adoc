...
* `BMM_REFERENCEABLE`: a model element referencable by name within a scope and having a signature;
* `BMM_ROUTINE`: a model element whose definition is a compututation;
* `BMM_VALUED`: any entity that has a value at execution time;
...
All formally defined model elements (i.e. instances of the meta-type `BMM_REFERENCEABLE`) have a _scope_, represented by the `_scope_` meta-attribute. The scope meta-type indicates the kind of model element within which the current element is scoped, and the value 
...
Functions and procedures are kinds of _routine_, which are computational features of a class. In BMM, they are represented, along with constituent parts, namely parameters and local variables by descendants of the class `BMM_REFERENCEABLE`.
...
include::{uml_export_dir}/classes/bmm_referenceable.adoc[]
include::{uml_export_dir}/classes/bmm_valued.adoc[]
