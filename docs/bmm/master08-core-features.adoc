...
A typical example of properties of class is shown below, with the `_other_participations_` and `_links_` properties exemplifying the container property meta-type:
...
Any routine may have pre- and/or post-conditions defined for it, represented in BMM by `ASSERTIONs` populating the `_pre_conditions_` and `_post_conditions_` attributes of `BMM_ROUINE`. An Assertion is a tagged Boolean Expression, whose internal structure is described below in <<expressions>>.
