...
A special meta-type, `EL_SELF_REF` is provided to represent references to the current scoping object for expressions. And instance of this meta-class corresponds to references such as 'this' or 'self' found in object-oriented programming languages.
...
The following shows the comparison of a property value to a constant, where the `>=` operator maps to the function `Real._greater_than_or_equal_()`
