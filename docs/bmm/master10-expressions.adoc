...
A special meta-type, `EL_ANONYMOUS_REF` is provided for references to an anonymous instance created via reference to its class. This is exclusively used as a convenient way (common in many programming languages) of accessing _class features_ that do not depend on instance properties (i.e. 'constant', 'enum', and 'static' members in C-based languages) without having to explicitly create an instance. As an expression, an anonymous reference will accordingly only appear as a qualifier of a feature reference, using syntax such as `{ClassName}._feature_`, or with arguments, `{ClassName}._function (args...)_`.
...
Since an anonymous reference can be made without instantiation, it may be made to an abstract class as well as a concrete one. This facilitates the use of so-called 'utility' classes containing groups of useful functions e.g. for handling specific kinds of strings.

Examples of anonymous reference are:
...
The following abstract syntax examples illustrate expressions containing the various kinds of terminal reference.
...
The following is the same logical condition, but with the use of object context specified with typical 'dot notation', here `patient.`.
...
include::{uml_export_dir}/classes/el_anonymous_ref.adoc[]
