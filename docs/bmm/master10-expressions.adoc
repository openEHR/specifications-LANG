...
A special meta-type, `EL_ANONYMOUS_REF` is provided for references to an anonymous instance created via reference to its class. This is exclusively used as a convenient way (common in many programming languages) of accessing constants, enumeration values and class-level (i.e. 'static') functions without having to explicitly create an instance. As an expression, an anonymous reference will accordingly only appear as a qualifier of a feature reference, using syntax such as `{ClassName}._feature_`. A typical example of such use is `{Env}._current_time_` to access the class function `_current_time_` defined on the class (or interface) `Env`.
