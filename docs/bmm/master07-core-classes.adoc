...
A class defined within a model may include _invarants_, i.e. assertions relating to its state that hold true before and after all _public_ routine calls. Assertions are instances of the class `ASSERTION`, which is a tagged Boolean-valued Expression. The internal structure of the latter is defined below in <<expressions>>.
