...
* _effective_base_class_: a reference to the effective generating class; for defined types (`BMM_DEFINED_TYPE` meta-type), this will just be the defining class (`_defining_class_`); for open generic type parameters, the constraining class (`Any` if no other); and for container types, it is the defining class of the base type, i.e. the type of the contained values;
...
Unitary meta-types are further distinguished as formal generic parameters (`BMM_PARAMETER TYPE`), _tuple types_, and _defined_ types, i.e. types with class definitions. The first of these is used to represent replaceable formal generic parameters (typically 'T', 'U' within types such as `List<T>` etc) within generic type declarations. Tuple types are special constructs stated purely in terms of other types.

Defined types have as their meta-type `BMM_DEFINED_TYPE`, whose subtypes are `BMM_ENTITY_TYPE`, a meta-type for entities (i.e. full objects) and `BMM_SIGNATURE`, a meta-type representing signatures of routines and valued model elements (properties, variables etc). The former divides into `BMM_SIMPLE_TYPE` and `BMM_GENERIC_TYPE`, corresponding to the standard notions of type familiar in modern programming languages. The class definitions of instances (i.e. BMM model class definitions) of these meta-types are available via the property `_defining_class_`, of meta-type `BMM_CLASS` for a BMM simple type, and `BMM_GENERIC_CLASS` for BMM generic type.
