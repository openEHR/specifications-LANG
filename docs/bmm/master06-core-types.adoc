...
One requirement for a formalism to support functional expressions is to provide a representation for the type of a _routine call_, and thus of a _function call_. Such types take the form of signatures, and are represented by the meta-type `BMM_SIGNATURE`. Instances of this meta-type represent any kind of signature, and consist of `_argument_types_`, whose meta-type is `BMM_TUPLE_TYPE` (See <<Tuple Type>>), and `_result_type_`, representing the result type.

The same meta-type is used to represent signatures of both procedures and functions, even though procedures have no result type. This is achieved using invariants, as described in <<_signatures>>.
