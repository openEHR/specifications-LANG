...
In EL, a _decision table_ is a complex expression that expresses the equivalent logic as an if/then/else chain or case statement that returns values (i.e. not one whose statements are procedure calls). 

The syntax takes a standard form and a compact form. The standard form is as follows, and is used for multi-branch decision structures.
