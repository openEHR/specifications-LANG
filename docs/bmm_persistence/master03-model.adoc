...
The `p_nmm.persistence` package, shown below, defines a simplified form of the main BMM model suitable for persisting and human authoring. The {openehr_its_bmm_releases}[openEHR BMM schemas] are authored in the `P_BMM` form of the BMM, using the {openehr_odin}[openEHR ODIN syntax].

[.text-center]
.`base.p_bmm.persistence` Package
image::{uml_diagrams_uri}/LANG-p_bmm.persistence.svg[id=p_bmm_persistence_package, align="center"]
